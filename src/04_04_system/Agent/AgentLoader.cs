...
namespace FourthDevs.System.Agent
{
    ...
    /// Each file has YAML-like frontmatter (delimited by ---) with title, model, tools,
    /// and description, followed by the system prompt as Markdown.
    ///
    /// Mirrors 04_04_system/src/loader.js (i-am-alice/4th-devs).
    /// </summary>
    internal static class AgentLoader
    {
        private static readonly string AgentsDir =
            Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "workspace", "system", "agents");

        /// <summary>
        /// Reads and parses <c>workspace/system/agents/{name}.md</c>.
        /// </summary>
        public static AgentTemplate Load(string name)
        {
            string filePath = Path.Combine(AgentsDir, name + ".md");
