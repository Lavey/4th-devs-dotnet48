...
namespace FourthDevs.AgentGraph.Memory
{
    ...
    public static class Observer
    {
        ...
        public static string SerializeItems(List<Item> items, Dictionary<string, string> actorNames)
        {
            ...
            {
                string actor = "system";
                if (item.ActorId != null && actorNames.ContainsKey(item.ActorId))
                    actor = actorNames[item.ActorId];

                switch (item.Type)
                {
                    case "message":
                        sb.AppendLine(string.Format("[{0}] {1}", actor,
                    ...
                    case "invocation":
                        ...
                        break;
                    case "result":
                        sb.AppendLine(string.Format("[{0}] {1} → {2}", actor,
                            item.Content != null && item.Content["tool"] != null ? item.Content["tool"].ToString() : "",
...
Primary: ...
        ...
        public static async Task<ObserverResult> RunObserver(string previousObservations, List<Item> items, Dictionary<string, string> actorNames)
        {
            var history = SerializeItems(items, actorNames);
