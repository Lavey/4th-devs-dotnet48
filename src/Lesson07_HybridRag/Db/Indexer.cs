...
namespace FourthDevs.Lesson07_HybridRag.Db
{
    ...
    ///
    /// Mirrors 02_02_hybrid_rag/src/db/indexer.js (i-am-alice/4th-devs)
    /// </summary>
    internal static class Indexer
    {
        ...
        /// <summary>
        /// Index all supported files in <paramref name="workspacePath"/>.
        /// Skips unchanged files (by SHA-256 hash) and removes stale entries.
        /// </summary>
            ...
            SQLiteConnection db, string workspacePath)
        {
            ...
            Console.WriteLine(
                string.Format("[indexer] Found {0} file(s)", supported.Count));

            using (var embedder = new EmbeddingClient())
            {
                foreach (string filePath in supported)
                    await IndexFileAsync(db, embedder, filePath,
            ...
            var indexed = SelectAll(db,
                "SELECT id, source FROM documents", r =>
        ...
        // ----------------------------------------------------------------
        // Chunking (separator-based, same as Lesson07_Chunking)
        // ----------------------------------------------------------------
